...
def get_model(model_name):
    if model_name not in _models:
        _models[model_name] = spacy.load("model")
...
class EntResource(object):
    ...
    def on_post(self, req, resp):
        ...
        try:
            ...
            doc = nlp(text)
            resp.body = spacy.displacy.render(doc, style='ent')
...
class ClassResource(object):
    ...
    def on_post(self, req, resp):
        ...
        try:
            nlp = get_model('en_core_web_sm')
            doc = nlp(text)
            resp.body = json.dumps(doc.cats, sort_keys=True,
                                   indent=2)
            resp.content_type = 'text/string'           
            resp.status = falcon.HTTP_200
        except Exception as e:
...
APP.add_route('/cls', ClassResource())
